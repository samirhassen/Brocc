...
import { PositiveCreditRegisterModule } from './ul-legacy-credit/positive-credit-register/positive-credit-register.module';
...
export function initializeConfig(configService: ConfigService, httpClient: HttpClient, platform: PlatformLocation) {
    return () => {
        return loginManager.onInit(httpClient, platform).then((x) => {
            if (!x.wasRedirected) {
                ...
                return new Promise<Object>((resolve) => resolve(null));
            }
        });
    };
}
