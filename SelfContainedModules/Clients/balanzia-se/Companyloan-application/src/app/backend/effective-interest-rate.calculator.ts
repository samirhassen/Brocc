...
export class EffectiveInterestRateCalculation {
    ...
    calculate(): NullableNumber {
        if (this.loans.length == 1 && this.payments.length == 1 && NTechMath.equals(this.loans[0][0], this.payments[0][0])) {
    ...
    private findRootUsingBrentsMethod(
        ...
        let c: number, d: number, e: number, fa: number, fb: number, fc: number, tol: number, m: number, p: number, q: number, r: number, s: number;
        ...
        while (true) {
            ...
            if (Math.abs(m) + Number.EPSILON > tol && !NTechMath.equals(fb, 0.0)) {// exact comparison with 0 is OK here
                ...
                if (Math.abs(e) < tol + Number.EPSILON || Math.abs(fa) <= Math.abs(fb) + Number.EPSILON) {
                    ...
                    if (p + Number.EPSILON > 0.0)
                        q = -q;
                    else
                        p = -p;
                    s = e; e = d;
                    if (2.0 * p < 3.0 * m * q - Math.abs(tol * q) + Number.EPSILON && p < Math.abs(0.5 * s * q) + Number.EPSILON)
                        d = p / q;
                    else
                        d = e = m;
                }
                a = b; fa = fb;
                if (Math.abs(d) + Number.EPSILON > tol)
                    b += d;
                else if (m + Number.EPSILON > 0.0)
                    b += tol;
                else
                    b -= tol;
                ...
                fb = f(b);
                if ((fb + Number.EPSILON > 0.0 && fc + Number.EPSILON > 0.0) || (fb <= 0.0 + Number.EPSILON && fc <= 0.0 + Number.EPSILON))
                    skipInt = false
                else
                    skipInt = true
            }
            else
                return new NullableNumber(b);
        }
    }
}