        ...
        private string CreateFile(TreasuryDomainModel model, DateTime deliveryDate)
        {
            ...
            using (var fs = System.IO.File.CreateText(tempFileName))
            {
                ...
                if (string.IsNullOrWhiteSpace(f.Opt("ProductTypeSavingsAccount")))
                    throw new System.InvalidOperationException("ProductTypeSavingsAccount is missing in " + file.FullName);
                if (string.IsNullOrWhiteSpace(f.Opt("PrefixSavingsCustomerId")))
                    throw new System.InvalidOperationException("PrefixSavingsCustomerId is missing in " + file.FullName);
                if (string.IsNullOrWhiteSpace(f.Opt("PrefixSavingsAccountNo")))
                    throw new System.InvalidOperationException("PrefixSavingsAccountNo is missing in " + file.FullName);

                fs.WriteLine("Reporting Date" + sep + "Product type" + sep + "General ledger account" + sep + "Customer Id" + sep + "Customer full name" + sep + "Customer country" + sep +
                    "Account number" + sep + "Current balance" + sep + "Currency" + sep + "Account creation date" + sep + "Current interest rate");
