...
public class RiksgaldenFileExporter(DateTime writeDate, string instituteName, string instituteOrgnr)
{
    ...
    public void WriteTransactionsFileToStream(IList<Transaction> transactions, bool isInitialTransactionFile,
        Stream target)
    {
    ...
    public void WriteFinnishFilialFileToStream(IList<FiFilialCustomer> customers, IList<FiFilialAccount> accounts,
        Stream target)
    {
        ...
        string ClipRight(string s, int maxLength) => (s ?? "").Length > maxLength ? s.Substring(0, maxLength) : s;
    }
}