...
namespace nSavings.Code.Riksgalden
{
    public class RiksgaldenConfiguration
    {
        ...
        public static RiksgaldenConfiguration Create(ClientConfiguration clientConfig)
        {
            var s = clientConfig.GetCustomSection("SavingsRiksgalden");
            var instituteName = s?.Descendants()?.SingleOrDefault(x => x.Name.LocalName == "DepositorGuaranteeInstituteName")?.Value;
            var instituteOrgnr = s?.Descendants()?.SingleOrDefault(x => x.Name.LocalName == "DepositorGuaranteeOrgnr")?.Value;

            if (string.IsNullOrWhiteSpace(instituteName))
                throw new Exception("Missing required client config value SavingsRiksgalden.DepositorGuaranteeInstituteName");
