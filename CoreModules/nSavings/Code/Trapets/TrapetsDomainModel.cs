...
public class TrapetsDomainModel
{
    ...
    private class CustomerCivicRegNrCache(CustomerClient client, CivicRegNumberParser civicRegNumberParser)
    {
        private readonly Dictionary<int, ICivicRegNumber> civicRegNrByCustomerId = new();
        ...
        var q = context
            ...
            .Where(x => x.SavingsAccountNr != null &&
                        ((x.OutgoingPaymentId.HasValue &&
                          x.AccountCode == nameof(LedgerAccountTypeCode.ShouldBePaidToCustomer) &&
                          x.BusinessEvent.EventType == nameof(BusinessEventType.OutgoingPaymentFileExport)) ||
