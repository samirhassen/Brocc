...
namespace nSavings.Code.Trapets
{
    public class TrapetsDomainModel
    {
        ...
        private class CustomerCivicRegNrCache
        {
            ...
            private CivicRegNumberParser civicRegNumberParser;
            ...
            private Dictionary<int, ICivicRegNumber> civicRegNrByCustomerId = new Dictionary<int, ICivicRegNumber>();
        ...
        private static List<KycQuestionAnswerModel> GetQuestionsAndAnswersWithChangesAfter(byte[] lastSeenTimestamp, out byte[] maxTimestamp, SavingsContext context, TrapetsKycConfiguration config, Func<ISet<int>, IDictionary<int, ICivicRegNumber>> getCivicRegNumbers)
        {
            const string SavingsQuestionPrefix = "savings_";

            Func<string, string> stripSavingsPrefix = x => x.StartsWith(SavingsQuestionPrefix) ? x.Substring(SavingsQuestionPrefix.Length) : x;
        ...
        private static List<CustomerModel> GetCustomersWithChangesAfter(byte[] latestSeenTimestamp, out byte[] maxTimestamp, SavingsContext context, CustomerClient customerClient, CustomerCivicRegNrCache civicRegNrs)
        {
            ...
            var customerData = customerClient.FetchTrapetsAmlData(latestSeenTimestamp, savingaAccountNrsByCustomerId.Keys.ToList());
            ...
            foreach (var c in customerData.Item2)
            {
                ...
                {
                    Item = c,
                    SavingsAccounts = savingaAccountNrsByCustomerId[c.CustomerId].Select(x => new CustomerModel.SavingsCustomer
                    {
                        IsMainCustomer = true,
                        SavingsAccountNr = x,
                        NrOfCustomers = 1
        ...
        private static List<TransactionModel> GetTransactionsAfter(byte[] lastSeenTimestamp, out byte[] maxTimestamp, SavingsContext context)
        {
            var q = context
                .LedgerAccountTransactions
                .AsNoTracking()
                .Where(x => x.SavingsAccountNr != null && ((x.OutgoingPaymentId.HasValue && x.AccountCode == LedgerAccountTypeCode.ShouldBePaidToCustomer.ToString() && x.BusinessEvent.EventType == BusinessEventType.OutgoingPaymentFileExport.ToString()) || (x.IncomingPaymentId.HasValue && x.AccountCode == LedgerAccountTypeCode.Capital.ToString())))
                .AsQueryable();
        ...
        private static List<AccountModel> GetAccountsWithChangesAfter(byte[] lastSeenTimestamp, out byte[] maxTimestamp, SavingsContext context)
        {
            var q = context
                .SavingsAccountHeaders
                .AsNoTracking()
        ...
        private static List<AssetModel> GetAssetsWithChangesAfter(byte[] lastSeenTimestamp, out byte[] maxTimestamp, SavingsContext context)
        {
            var q = context
                .SavingsAccountHeaders
                .AsNoTracking()
            ...
            {
                x.SavingsAccountNr,
                CapitalBalanceTransactions = x.Transactions.Where(y => y.AccountCode == LedgerAccountTypeCode.Capital.ToString())
            })
            .Select(x => new
            {
                x.SavingsAccountNr,
                CapitalBalance = x.CapitalBalanceTransactions.Sum(y => y.Amount),
                Timestamp = x.CapitalBalanceTransactions.OrderByDescending(y => y.Timestamp).Select(y => y.Timestamp).FirstOrDefault(),
                TransactionDate = x.CapitalBalanceTransactions.OrderByDescending(y => y.TransactionDate).Select(y => y.TransactionDate).FirstOrDefault(),
            });
