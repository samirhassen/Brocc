...
{
    public class FatcaExportService : IFatcaExportService
    {
        ...
        public FatcaExportService(IClock clock, Func<string, string> getUserDisplayNameByUserId, UrlHelper urlHelper)
        {
            this.clock = clock;
        ...
        public void CreateFatcaFileToStream(DateTime forYearDate, Stream target, Action<int> observeNrOfAccounts = null)
        {
            ...
            var fatcaRequest = new CustomerClient.CreateFatcaExportFileRequest
            {
                ...
                ExportDate = this.clock.Now.DateTime,
                Accounts = new List<CustomerClient.FatcaExportFileRequestAccount>()
            };
            ...
            string exportProfileName)
        {
            ...
            {
                Status = OutgoingExportFileHeader.StandardExportResultStatusModel.StatusCode.NoExportProfile.ToString(),
                FailedProfileNames = new List<string>(),
                SuccessProfileNames = new List<string>(),
            ...
            r.Status = r.Errors.Any()
                ? OutgoingExportFileHeader.StandardExportResultStatusModel.StatusCode.Error.ToString()
                : ((r.Warnings.Any() || r.FailedProfileNames.Any())
                    ? OutgoingExportFileHeader.StandardExportResultStatusModel.StatusCode.Warning.ToString()
                    : OutgoingExportFileHeader.StandardExportResultStatusModel.StatusCode.Ok.ToString());
