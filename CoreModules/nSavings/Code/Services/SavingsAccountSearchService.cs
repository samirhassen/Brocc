...
public class SavingsAccountSearchService(ICustomerClient customerClient)
{
    public List<SavingsAccountSearchHit> Search(SavingsAccountSearchRequest request)
    {
        ...
        using (var context = new SavingsContext())
        {
            ...
            {
                allHits.AddRange(
                    CreateResultFromAccountsQueryable(
                        context.SavingsAccountHeaders.Where(x => x.SavingsAccountNr == searchQuery)));
            }

            if (!request.SkipCustomerSearch)
            {
                var customerIds = customerClient.FindCustomerIdsOmni(searchQuery);
                var hits = CreateResultFromAccountsQueryable(
                    context.SavingsAccountHeaders.Where(x => customerIds.Contains(x.MainCustomerId)));
                allHits.AddRange(hits);
            }
        }
