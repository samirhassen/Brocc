...
public class CustomerClient : AbstractServiceClient
{
        ...
        IList<int> customerIds)
    {
        ...
        if (rr.Items is not { Count: > 0 }) return Tuple.Create((byte[])null, new List<TreasuryAmlItem>());
        ...
        IList<int> customerIds)
    {
        ...
        if (rr.Items is not { Count: > 0 }) return Tuple.Create((byte[])null, new List<TrapetsAmlItem>());
