        ...
        protected override ActionResult.FileStream DoExecuteFileStream(NTechWebserviceMethodRequestContext requestContext, Request request)
        {
            ...
            using (var context = new SavingsContext())
            {
                accountNrsByCustomerId = context.SavingsAccountHeaders.Select(x => new
                {
                    ...
                    AccountStatus = x
                            .DatedStrings
                            .Where(y => y.Name == DatedSavingsAccountStringCode.SavingsAccountStatus.ToString() && y.TransactionDate <= forDate)
            ...
            var customerPropertiesByCustomerId = customerClient.BulkFetchPropertiesByCustomerIdsSimple(accountNrsByCustomerId.Keys.ToHashSet(),
            ...
            return ExcelFile(result, downloadFileName: $"AmlReportingAid-Savings-FI-{forDate.ToString("yyyy-MM-dd")}.xlsx");
        }
