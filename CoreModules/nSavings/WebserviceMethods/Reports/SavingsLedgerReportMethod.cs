            ...
            NTechWebserviceMethodRequestContext requestContext, Request request)
        {
            ...
            using (var context = new SavingsContext())
            {
                ...
                var result = accountsBase
                    ...
                    .Select(x => new
                    {
                        ...
                        ClosedDate = x.H
                            ...
                            .FirstOrDefault(),
                        FixedAccountProduct = context.FixedAccountProducts
                            .Where(y => y.Id == x.H.FixedInterestProduct)
                            .OrderByDescending(y => y.Name)
                            .Select(y => y.InterestRatePercent)
                            .FirstOrDefault(),
                ...
                s.SetColumnsAndData(result, SkipNulls(
                    ...
                    result.Col(x => x.MaturesAt, ExcelType.Date, HeaderText("Matures At")),
                    result.Col(x => (decimal?)x.FixedAccountProduct, ExcelType.Number, "Fixed Account Product"),
                    !includeCustomerDetails
                        ? null
