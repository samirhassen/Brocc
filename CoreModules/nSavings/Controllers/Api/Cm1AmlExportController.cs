...
using Serilog;
using DocumentClient = nSavings.Code.DocumentClient;
...
namespace nSavings.Controllers.Api
{
    ...
    public class ApiCm1AmlExportController : NController
    {
        ...
        private ActionResult CreateExportI(IDictionary<string, string> schedulerData)
        {
            ...
            try
            {
                using (var context = new SavingsContext())
                {
                    ...
                    var file = NTechEnvironment.Instance.StaticResourceFile("ntech.credit.cm1.settingsfile",
                        "cm1-business-credit-settings.txt", false);
                    if (!file.Exists)
                        throw new InvalidOperationException(
                            "The configuration file ntech.credit.cm1.settingsfile is missing!");
                    ...
                    var model = SavingsCm1DomainModel.GetChangesSinceLastExport(CurrentUserId, InformationMetadata,
                        resolver.ContextFactory,
                        new Lazy<NTechSimpleSettingsCore>(() => f),
                        customerClient,
                        ControllerServiceFactory.GetEncryptionService(GetCurrentUserMetadata()));
                    var cmlExportFileResponse = customerClient.CreateCm1AmlExportFiles(
                        new PerProductCmlExportFileRequest
                        {
                            Savings = true,
                    ...
                    var transactionFiles =
                        new List<OutgoingAmlMonitoringExportFileHeader>();
        ...
        private OutgoingAmlMonitoringExportFileHeader CreateAndAddExportHeader(SavingsContext context,
            string archiveKey)
        {
        ...
        private (List<string> Errors, List<string> Warnings) DeliverCm1Files(SavingsContext context,
            List<OutgoingAmlMonitoringExportFileHeader> transactionFiles,
            ...
            DocumentClient documentClient)
        {
            ...
            void DeliverFile(OutgoingAmlMonitoringExportFileHeader file, string profileName)
            {
                ...
                {
                    deliveryResult = DeliverExportFile(file.FileArchiveKey, fileErrors, documentClient, null,
                        profileName);
                }
                else
                ...
                if (deliveryResult?.DeliveredToProfileName == null)
                {
                    ...
                    fileWarnings.Add(skipDeliveryExport
                        ? "File not exported due to schedulerData override"
                        : "File not exported but was expected to be");
                }
