...
namespace nSavings.Controllers.Api.Reports
{
    ...
    {
        [HttpGet, Route("Api/Reports/GetProviderFeedback")]
            ...
            try
            {
                ...
                using (var context = new SavingsContext())
                {
                    var accountsBase = context
                        ...
                        .Where(x => x.CreatedByEvent.TransactionDate >= fromDate &&
                                    x.CreatedByEvent.TransactionDate <= toDate)
                        .Select(x => new
                        {
                            x.SavingsAccountNr,
                    ...
                    var keys = new HashSet<string>(accountsBase.Where(x => x.ExternalVariablesKey != null)
                        .Select(x => x.ExternalVariablesKey).ToList());
                    if (keys.Any())
                    {
                        var result = Service.KeyValueStore(GetCurrentUserMetadata()).GetValues(keys,
                        ...
                        externalVariablesByKey = keys
                            ...
                            {
                                x.Key,
                                Variables = JsonConvert
                                    .DeserializeAnonymousType(x.Value, new[] { new { Name = "", Value = "" } })
                    ...
                    var accounts = accountsBase.Select(x => new
                        {
                            ...
                            x.SavingsAccountNr,
                            ExternalVariables =
                                (x.ExternalVariablesKey != null
                                    ? externalVariablesByKey.Opt(x.ExternalVariablesKey)
                        ...
                        .Select(x => new
                        {
                            x.CreatedDate,
                            x.SavingsAccountNr,
                            ProviderName = x.ExternalVariables.Where(y => providerNameVariables.Contains(y.Key))
                                .Select(y => y.Value).FirstOrDefault(),
                            CampaignName = x.ExternalVariables.Where(y => campaignNameVariables.Contains(y.Key))
                                .Select(y => y.Value).FirstOrDefault(),
                            OtherVariables = x.ExternalVariables.Where(y => !allPredefinedVariables.Contains(y.Key))
                    ...
                    var sheets = new List<DocumentClientExcelRequest.Sheet>
                    {
                        new DocumentClientExcelRequest.Sheet
                        {
                            ...
                            Title = $"{fromDate:yyyy-MM-dd} {toDate:yyyy-MM-dd}"
                        }
                    };
                    ...
                    {
                        FileDownloadName =
                            $"savingsProviderFeedback-{fromDate:yyyy-MM-dd}-{fromDate:yyyy-MM-dd}.xlsx"
                    };
                }
            }
            catch (Exception ex)
