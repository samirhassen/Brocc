...
.controller('loadingApp', ['$scope', 'sharedService', function ($scope, sharedService) {
    ...
    $scope.$watch(
        function () { return sharedService.isLoading },
        function () {
            $scope.isLoading = sharedService.isLoading
        }
    )
...
.controller('backApp', ['$scope', 'sharedService', function ($scope, sharedService) {
    $scope.$watch(
        function () { return sharedService.backUrl },
        function () {
    ...
    $scope.verifyPlace = function (evt) {
        ...
        $http({
            ...
            if (d.isOk) {
                ...
                sharedService.gotoPlace(
                    d.savingsAccountNr,
                    $scope.placeAmount,
                    $scope.allowOverMaxAllowedSavingsCustomerBalance,
                    function (failedMsg) { $scope.placeFailedMessage = failedMsg },
                    function () { $scope.placeFailedMessage = null })
            } else {
