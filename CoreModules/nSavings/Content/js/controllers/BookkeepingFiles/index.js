...
app
    .controller('ctr', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
        ...
        $scope.rulesAsXlsUrl = initialData.rulesAsXlsUrl;
        $scope.exportProfileName = initialData.exportProfileName;
        function today() {
            return moment(initialData.today, 'YYYY-MM-DD', true);
        }
        $scope.fromCreatedDate = today().add(-10, 'days').format('YYYY-MM-DD');
        $scope.toCreatedDate = today().format('YYYY-MM-DD');
        ...
        $scope.createFile = function (evt) {
            ...
            $http({
                ...
                if (response.data.noNewTransactions) {
                    toastr.warning('No new transactions exist');
        ...
        $scope.gotoPage(0, { FromDate: $scope.fromCreatedDate, ToDate: $scope.toCreatedDate });
        window.scope = $scope;
    }]);
