...
public partial class SavingsContext : ChangeTrackingDbContext
{
    private const string ConnectionStringName = "SavingsContext";

    public static bool IsConcurrencyException(Exception ex) =>
        ex.GetType().FullName == typeof(DbUpdateConcurrencyException).FullName;
    ...
    public virtual DbSet<FixedAccountProduct> FixedAccountProducts { get; set; }
    public virtual DbSet<FixedAccountProductAuditLog> FixedAccountProductAuditLog { get; set; }
    ...
    protected override void OnModelCreating(DbModelBuilder modelBuilder)
    {
        ...
        Cfg<BusinessEvent>(modelBuilder, ch =>
        {
            ...
            ch.HasMany(x => x.CreatedLedgerTransactions).WithRequired(x => x.BusinessEvent)
                .HasForeignKey(x => x.BusinessEventId);
            ch.HasMany(x => x.CreatedSavingsAccounts).WithRequired(x => x.CreatedByEvent)
                .HasForeignKey(x => x.CreatedByBusinessEventId);
            ch.HasMany(x => x.CreatedDatedSavingsAccountStrings).WithRequired(x => x.BusinessEvent)
                .HasForeignKey(x => x.BusinessEventId);
            ch.HasMany(x => x.CreatedDatedSavingsAccountValues).WithRequired(x => x.BusinessEvent)
                .HasForeignKey(x => x.BusinessEventId);
            ch.HasMany(x => x.CreatedSharedDatedValues).WithRequired(x => x.BusinessEvent)
                .HasForeignKey(x => x.BusinessEventId);
            ch.HasMany(x => x.CreatedSavingsAccountCreationRemarks).WithRequired(x => x.CreatedByEvent)
                .HasForeignKey(x => x.CreatedByBusinessEventId);
            ch.HasMany(x => x.CreatedSharedSavingsInterestRates).WithRequired(x => x.BusinessEvent)
                .HasForeignKey(x => x.BusinessEventId);
            ch.HasMany(x => x.CreatedIncomingPaymentFiles).WithRequired(x => x.CreatedByEvent)
                .HasForeignKey(x => x.CreatedByBusinessEventId);
            ch.HasMany(x => x.CreatedSavingsAccountInterestCapitalizations).WithRequired(x => x.CreatedByEvent)
                .HasForeignKey(x => x.CreatedByBusinessEventId);
            ch.HasMany(x => x.CreatedOutgoingPayments).WithRequired(x => x.CreatedByEvent)
                .HasForeignKey(x => x.CreatedByBusinessEventId);
            ch.HasMany(x => x.CreatedOutgoingPaymentFiles).WithRequired(x => x.CreatedByEvent)
                .HasForeignKey(x => x.CreatedByBusinessEventId);
            ch.HasMany(x => x.InitiatedSavingsAccountWithdrawalAccountChanges).WithRequired(x => x.InitiatedByEvent)
                .HasForeignKey(x => x.InitiatedByBusinessEventId);
            ch.HasMany(x => x.CreatedSavingsAccountKycQuestions).WithRequired(x => x.BusinessEvent)
                .HasForeignKey(x => x.BusinessEventId);
            ch.HasMany(x => x.CreatedSavingsAccountDocuments).WithRequired(x => x.CreatedByEvent)
                .HasForeignKey(x => x.CreatedByBusinessEventId);
            ch.HasMany(x => x.SharedSavingsInterestRateChangeRemovals).WithOptional(x => x.RemovedByBusinessEvent)
                .HasForeignKey(x => x.RemovedByBusinessEventId);
            ch.HasMany(x => x.SharedSavingsInterestRateAppliesToAccountsSinces)
                .WithOptional(x => x.AppliesToAccountsSinceBusinessEvent)
                .HasForeignKey(x => x.AppliesToAccountsSinceBusinessEventId);
            ch.HasMany(x => x.CreatedSharedSavingsInterestRateChangeHeaders).WithRequired(x => x.BusinessEvent)
                .HasForeignKey(x => x.BusinessEventId);
            ch.HasMany(x => x.CommittedOrCancelledSavingsAccountWithdrawalAccountChanges)
                .WithOptional(x => x.CommitedOrCancelledByEvent).HasForeignKey(x => x.CommitedOrCancelledByEventId);
        });
        ...
        Cfg<DatedSavingsAccountString>(modelBuilder, c =>
        {
            ...
            c.Property(x => x.Value).IsRequired()
                .HasMaxLength(
                    100); //NOTE: Dont make this longer if longer values appear. Put them somewhere else. This needs to be indexable
            c.Property(x => x.TransactionDate).IsRequired().HasColumnType("date");
        });
        ...
        Cfg<SavingsAccountHeader>(modelBuilder, ch =>
        {
            ...
            ch.HasMany(x => x.Transactions).WithOptional(x => x.SavingsAccount)
                .HasForeignKey(x => x.SavingsAccountNr);
            ch.HasMany(x => x.DatedValues).WithRequired(x => x.SavingsAccount)
                .HasForeignKey(x => x.SavingsAccountNr);
            ch.HasMany(x => x.DatedStrings).WithRequired(x => x.SavingsAccount)
                .HasForeignKey(x => x.SavingsAccountNr);
            ch.HasMany(x => x.Comments).WithRequired(x => x.SavingsAccount).HasForeignKey(x => x.SavingsAccountNr);
            ch.HasMany(x => x.CreationRemarks).WithRequired(x => x.SavingsAccount)
                .HasForeignKey(x => x.SavingsAccountNr);
            ch.HasMany(x => x.SavingsAccountInterestCapitalizations).WithRequired(x => x.SavingsAccount)
                .HasForeignKey(x => x.SavingsAccountNr);
            ch.HasMany(x => x.SavingsAccountWithdrawalAccountChanges).WithRequired(x => x.SavingsAccount)
                .HasForeignKey(x => x.SavingsAccountNr);
            ch.HasMany(x => x.SavingsAccountKycQuestions).WithRequired(x => x.SavingsAccount)
                .HasForeignKey(x => x.SavingsAccountNr);
        });
        ...
        Cfg<SharedSavingsInterestRate>(modelBuilder, c =>
        {
            ...
            c.HasMany(x => x.AllAccountsHeaders).WithOptional(x => x.AllAccountsRate)
                .HasForeignKey(x => x.AllAccountsRateId);
            c.HasMany(x => x.NewAccountsHeaders).WithOptional(x => x.NewAccountsOnlyRate)
                .HasForeignKey(x => x.NewAccountsOnlyRateId);
        });
        ...
        Cfg<IncomingPaymentFileHeader>(modelBuilder, ch =>
        {
            ...
            ch.HasMany(x => x.Payments).WithOptional(x => x.IncomingPaymentFile)
                .HasForeignKey(x => x.IncomingPaymentFileId);
        });
        ...
        Cfg<IncomingPaymentHeader>(modelBuilder, ch =>
        {
            ...
            ch.HasMany(x => x.Transactions).WithOptional(x => x.IncomingPayment)
        ...
        Cfg<SavingsAccountInterestCapitalization>(modelBuilder, ch =>
        {
            ...
            ch.Property(x => x.ToDate).IsRequired().HasColumnType("date").HasColumnAnnotation(
        ...
        Cfg<OutgoingPaymentHeader>(modelBuilder, ch =>
        {
            ...
            ch.HasMany(x => x.Transactions).WithOptional(x => x.OutgoingPayment)
                .HasForeignKey(x => x.OutgoingPaymentId);
        });
        ...
        Cfg<OutgoingBookkeepingFileHeader>(modelBuilder, ch =>
        {
            ...
            ch.HasMany(x => x.Transactions).WithOptional(x => x.OutgoingBookkeepingFile)
                .HasForeignKey(x => x.OutgoingBookkeepingFileHeaderId);
        });
        ...
        Cfg<FixedAccountProduct>(modelBuilder, c =>
        {
            ConfigureInfrastructureFields(c);
            c.HasKey(x => x.Id);

            c.Property(p => p.Name).IsRequired().HasMaxLength(100);
            c.Property(p => p.InterestRatePercent).IsRequired();
            c.Property(p => p.ValidFrom).IsRequired().HasColumnType("date");
            c.Property(p => p.ValidTo).HasColumnType("date");

            c.Property(e => e.CreatedAt).IsRequired().HasColumnType("date");
            c.Property(e => e.CreatedBy).IsRequired();
        });

        Cfg<FixedAccountProductAuditLog>(modelBuilder, c =>
        {
            c.HasKey(x => x.Id);
            c.Property(p => p.Message).IsRequired().HasMaxLength(512);
            c.Property(p => p.User).IsRequired();
            c.Property(e => e.CreatedAt).IsRequired().HasColumnType("date");
        });

        return;

        IndexAnnotation Index(bool isUnique) => new(new IndexAttribute { IsUnique = isUnique });
    }
    ...
    public static void InitDatabase()
    {
        Database.SetInitializer(
            new MigrateDatabaseToLatestVersion<SavingsContext, Configuration>());
        ...
        using (var context = new SavingsContext())
        {
            var _ = context.EncryptedValues.Any();
        }
    }
    ...
    public static T RunWithExclusiveLock<T>(string lockName, Func<T> onLockAcquired, Func<T> onAlreadyLocked,
        TimeSpan? waitForLock = null, bool useSharedLockSemantics = false) =>
        NTechSemaphoreManager.RunWithExclusiveLock(lockName, onLockAcquired, onAlreadyLocked,
    ...
    public static async Task<T> RunWithExclusiveLockAsync<T>(string lockName,
        ...
        await NTechSemaphoreManager.RunWithExclusiveLockAsync(lockName, onLockAcquired, onAlreadyLocked,
            acquireTimeout: acquireTimeout, ct);
}