...
public class ChangeInterestRateBusinessEventManager(int userId, string informationMetadata, IClock clock)
    : BusinessEventManagerBase(userId, informationMetadata, clock)
{
    ...
    public static IQueryable<SharedInterestRateModel> GetActiveInterestRates(ISavingsContext context)
    {
        return context
            .SharedSavingsInterestRatesQueryable
        ...
        out string failedMessage)
    {
        ...
        {
            if (s is not { RemovedByBusinessEventId: null } || s.ValidFromDate <= today) return;
