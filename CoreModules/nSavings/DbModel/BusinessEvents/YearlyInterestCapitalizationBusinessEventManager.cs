...
public class YearlyInterestCapitalizationBusinessEventManager(int userId, string informationMetadata, IClock clock)
    : BusinessEventManagerBase(userId, informationMetadata, clock)
{
    public void RunYearlyInterestCapitalization(bool includeCalculationDetails, out int successCount)
    {
        ...
        var eligibleAccountNrs = context
            ...
            .Where(x =>
                ...
                && x.AccountTypeCode == nameof(SavingsAccountTypeCode.StandardAccount)
        ...
        foreach (var accountNrGroup in eligibleAccountNrs.SplitIntoGroupsOfN(500))
        {
        ...
        Action<IDictionary<string, IList<ResultModel.DayResultModel>>> observeInterestAmountParts = null)
    {
        ...
        var allActiveAccounts = context
            ...
            .Select(x => new
            {
                ...
                Status = x
                    ...
                    .Where(y => y.TransactionDate <= historicalDate &&
                                y.Name == nameof(DatedSavingsAccountStringCode.SavingsAccountStatus))
            ...
            .Where(x => x.Status ==
                        nameof(SavingsAccountStatusCode
                            .Active)); // Don't need to filter on created date since this will do that implicitly
        ...
        bool skipNonActiveAccounts = false)
    {
        var capitalTransactions =
            context.LedgerAccountTransactions.Where(x => x.AccountCode == nameof(LedgerAccountTypeCode.Capital));
        ...
        {
            accounts = accounts.Where(x => x.Status == nameof(SavingsAccountStatusCode.Active)).ToList();
        }
        else
        {
            if (accounts.Any(x => x.Status != nameof(SavingsAccountStatusCode.Active)))
            {
        ...
        var models = accounts.Select(x =>
        {
            ...
            return new InputModel
            {
                ...
                LatestCapitalizationInterestFromDate = x.LatestInterestCapitalizationToDate.HasValue
                    ...
                    : null,
                OrderedCapitalTransactions = x.CapitalTransactions.Select(y =>
                    new InputModel.CapitalTransactionModel
                    {
                        Amount = y.Amount,
                        InterestFromDate = y.InterestFromDate
                    }).ToList(),
