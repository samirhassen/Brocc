...
namespace nTest.Controllers
{
    ...
    public class ApiCompanyLoanController : NController
    {
        ...
        public ActionResult GetTestCompany(string orgnr, string orgnrCountry, bool? generateIfNotExists)
        {
            ...
            var repo = new TestCompanyRepository(orgnrCountry, DbSingleton.SharedInstance.Db,
                CreateStoreHtmlDocumentInArchiveFunc());
        ...
        {
            var result = Enumerable.Range(1, count).Select(_ =>
                    GetOrGenerateTestCompanyI(null, null, isAccepted ?? true, seed, addToCustomerModule ?? false, null))
                .ToList();
        ...
        [HttpPost]
        public ActionResult GetOrGenerateTestCompany(string orgnr, string orgnrCountry, bool? isAccepted, int? seed,
            bool? addToCustomerModule, string bankAccountType)
        {
            var result = GetOrGenerateTestCompanyI(orgnr, orgnrCountry, isAccepted, seed, addToCustomerModule,
                bankAccountType);
            return Json2(result);
        }

        private GetOrGenerateTestCompanyResponseModel GetOrGenerateTestCompanyI(string orgnr, string orgnrCountry,
            bool? isAccepted, int? seed, bool? addToCustomerModule, string bankAccountType)
        {
            ...
            var orgnrParsed = orgnr == null
                ? new OrganisationNumberGenerator(country).Generate((x, y) => random.NextIntBetween(x, y - 1))
                : new OrganisationNumberParser(country).Parse(orgnr);

            var repo = new TestCompanyRepository(NEnv.ClientCfg.Country.BaseCountry, DbSingleton.SharedInstance.Db,
                CreateStoreHtmlDocumentInArchiveFunc());
            ...
            if (ep != null)
            {
                result = new GetOrGenerateTestCompanyResponseModel
                    { Orgnr = orgnrParsed.NormalizedValue, Properties = ep, WasGenerated = false };
            }
            else
            {
                ...
                var bankAccountNrTypeCode = !string.IsNullOrWhiteSpace(bankAccountType)
                    ? Enums.Parse<BankAccountNumberTypeCode>(bankAccountType).Value
                    : BankAccountNumberParser.GetDefaultAccountTypeByCountryCode(country);

                var pg = repo.GenerateNewTestCompany(isAccepted ?? true, random, now, overrides: addressOverride,
                    orgnr: orgnrParsed, reuseExisting: true, bankAccountNrType: bankAccountNrTypeCode);
            ...
            if (addToCustomerModule.GetValueOrDefault())
            {
                var res = c.CreateOrUpdateCompany(new CustomerClient.CreateOrUpdateCompanyRequest
                {
                    ...
                    Properties = result.Properties
                        .Where(x => !x.Key
                            .IsOneOf("orgnr", "orgnrCountry", "iban", "bankAccountNr", "bankAccountNrType"))
                        .Select(x => new CustomerClient.CreateOrUpdateCompanyRequest.Property
                        {
                            Name = x.Key,
                            Value = x.Value
                        }).ToList()
        ...
        [HttpPost]
        public ActionResult CreateOrUpdate(List<string> companies, bool clearCache = false)
        {
            var repo = new TestCompanyRepository(NEnv.ClientCfg.Country.BaseCountry, DbSingleton.SharedInstance.Db,
                CreateStoreHtmlDocumentInArchiveFunc());
            foreach (var company in companies ?? new List<string>())
            {
                ...
                if (!clearCache || !NEnv.ServiceRegistry.ContainsService("nCreditReport")) continue;
