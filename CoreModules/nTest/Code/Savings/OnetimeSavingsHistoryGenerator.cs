...
namespace nTest.Code
{
    public class OnetimeSavingsHistoryGenerator
    {
        public void Generate(Action<string> log)
        {
            ...
            try
            {
                ...
                while (currentSavingsDate < currentSystemTime)
                {
                    ...
                    //Morning
                    c.SetTime(new DateTimeOffset(currentSavingsDate.Year, currentSavingsDate.Month, currentSavingsDate.Day, 6, 0, 0, currentSavingsDate.Offset));

                    scheduler.TriggerTimeslot("Morning", schedulerData: schedulerData(isLastDayOfMonth));

                    //Midday
                    c.SetTime(new DateTimeOffset(currentSavingsDate.Year, currentSavingsDate.Month, currentSavingsDate.Day, 12, 0, 0, currentSavingsDate.Offset));

                    var newCount = r.NextIntBetween(0, 4);
                    if (newCount > 0)
                    {
                        var newAccounts = Enumerable.Range(1, newCount).ToList().Select(x => GenerateAccountTemplate(currentSavingsDate, r, p)).ToList();
                        foreach (var newAccount in newAccounts)
                        {
                            try
                            {
                                var cr = c.CreateSavingsAccount(g.CreateApplicationJson(newAccount.Owner, false, r, generateIban: true));
                            ...
                            {
                                log($"Failed to create account: {ex.ToString()}. Template: {JsonConvert.SerializeObject(newAccount)}");
                            }
                    ...
                    foreach (var a in accounts.Where(x => !x.WasCreatedFrozen() && x.Deposits.ContainsKey(currentSavingsDate.Date)))
                    {
                        var depositAmount = a.Deposits[currentSavingsDate.Date];
                        deposits.Add(new TestPaymentFileCreator.Payment
                        {
                            Amount = depositAmount,
                            BookkeepingDate = currentSavingsDate.Date.AddDays(r.ShouldHappenWithProbability(0.5m) ? -1 : 0),
                    ...
                    if (deposits.Any())
                    {
                        ...
                        c.ImportPaymentFile("camt.054.001.02", $"nTestIncomingPaymentFile_{currentSavingsDate.ToString("yyyy-MM-dd")}_{Guid.NewGuid().ToString()}.xml", paymentFileAsDataUrl, true);
                    }

                    //Withdrawals
                    foreach (var a in accounts.Where(x => !x.WasCreatedFrozen() && x.Withdrawals.ContainsKey(currentSavingsDate.Date)))
                    {
                    ...
                    c.SetTime(new DateTimeOffset(currentSavingsDate.Year, currentSavingsDate.Month, currentSavingsDate.Day, 18, 0, 0, currentSavingsDate.Offset));
        ...
        private AccountTemplate GenerateAccountTemplate(DateTimeOffset currentDate, IRandomnessSource r, TestPersonRepository p)
        {
            ...
            while (d <= t.ClosedDate)
            {
                ...
                d = d.AddDays(r.NextIntBetween(3, 90)); //Dont make the 3 shorter unless you want to handle a bunch of date stuff
            }
