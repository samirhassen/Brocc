...
namespace TestsnPreCredit.Fraud
{
    ...
    public class StandardFraudControlEngineTests
    {
        ...
        public void SameAddressCheck_OneCustomerWithMatchingAddress_ShouldReturnMatchingApplicationNr()
        {
            ...
            items.AddRange(ApplicationItemFactory.Create()
                .WithApplicants(matchingApplicationNr, matchingCustomerId, 1003));
            ...
            Assert.AreEqual(matchingApplicationNr,
                result.FraudControls.Single(x => x.CheckName == "SameAddressCheck").Values.Single());
        }
        ...
        public void SameEmailCheck_SameCustomerOnTwoApplications_ShouldNotShowMatchOnItself()
        {
            ...
            var items = new List<ComplexApplicationListItem>();
            items.AddRange(ApplicationItemFactory.Create()
                .WithApplicants(applicationNrToRunChecksFor, 1001, sameCustomerId));
        ...
        private Mock<DbSet<ComplexApplicationListItem>> GetMockedComplexApplicationListItemDbSet(
            List<ComplexApplicationListItem> items)
        {
            var asQueryable = items.AsQueryable();
            var mockedDbSet = new Mock<DbSet<ComplexApplicationListItem>>();

            mockedDbSet.As<IQueryable<ComplexApplicationListItem>>().Setup(m => m.Provider)
                .Returns(asQueryable.Provider);
            mockedDbSet.As<IQueryable<ComplexApplicationListItem>>().Setup(m => m.Expression)
                .Returns(asQueryable.Expression);
            mockedDbSet.As<IQueryable<ComplexApplicationListItem>>().Setup(m => m.ElementType)
                .Returns(asQueryable.ElementType);
            mockedDbSet.As<IQueryable<ComplexApplicationListItem>>().Setup(m => m.GetEnumerator())
                .Returns(asQueryable.GetEnumerator());

            return mockedDbSet;
        }

        private ComplexApplicationListItem CreateApplicationItem(string applicationNr, string listName, int nr,
    ...
    public static class ApplicationItemFactory
    {
        ...
        public static List<ComplexApplicationListItem> WithApplicants(this List<ComplexApplicationListItem> items,
            string applicationNr, params int[] customerIds)
        {
            var nr = 1;
        ...
        public static List<ComplexApplicationListItem> WithPaidToCustomerAccount(
            this List<ComplexApplicationListItem> items, string applicationNr, string bankAccountNr)
        {
            items.Add(CreateApplicationItem(applicationNr, "FinalLoanTerms", 1, "paidToCustomerBankAccountNr",
                bankAccountNr));
            return items;
        }

        private static ComplexApplicationListItem CreateApplicationItem(string applicationNr, string listName, int nr,
            string itemName, string itemValue)
        {
            return new ComplexApplicationListItem
            {
