...
namespace TestsnPreCredit.Savings
{
    ...
    public class SavingsTests
    {
        ...
        public void ExternalArticleTestCase1()
        {
            ...
            var capitalTransactions = new List<YearlyInterestCapitalizationBusinessEventManager.InputModel.CapitalTransactionModel>();

            Action<DateTime, decimal> deposit = (d, am) =>
                            capitalTransactions.Add(new YearlyInterestCapitalizationBusinessEventManager.InputModel.CapitalTransactionModel
                            {
                                Amount = am,
                                InterestFromDate = d.AddDays(1)
                            });
            Action<DateTime, decimal> withdrawal = (d, am) =>
                            capitalTransactions.Add(new YearlyInterestCapitalizationBusinessEventManager.InputModel.CapitalTransactionModel
                            {
                                Amount = -am,
                                InterestFromDate = d
            ...
            accounts.Add(new YearlyInterestCapitalizationBusinessEventManager.InputModel
            {
                SavingsAccountNr = "1",
                ...
                OrderedInterestRateChanges = new List<YearlyInterestCapitalizationBusinessEventManager.InputModel.InterestRateChangeModel>()
                {
                    new YearlyInterestCapitalizationBusinessEventManager.InputModel.InterestRateChangeModel
                    {
                        InterestRatePercent = 4m,
                        ValidFromDate = new DateTime(2008,1,1),
                    },
                    new YearlyInterestCapitalizationBusinessEventManager.InputModel.InterestRateChangeModel
                    {
            ...
            var ok = YearlyInterestCapitalizationBusinessEventManager.TryComputeInterestRateUntilDate(accounts, new DateTime(2008, 1, 31), true, out result, out failedMessage);
            ...
            foreach (var item in result.Values.Single().SummarizeParts())
            {
                ...
                var f = item.First();
                Console.WriteLine($"{fromDate.ToString("yyyy-MM-dd")}{(fromDate == toDate ? "" : "-" + toDate.ToString("yyyy-MM-dd"))}{(f.IsLeapYear ? "L" : "N")}: {item.Count()} x {f.AccountBalance.ToString("F2", c)} x {(f.AccountInterestRatePercent.HasValue ? (f.AccountInterestRatePercent.Value / 100m).ToString("P") : "-")} =>  {f.Amount.ToString("F2", c)}");
            }
