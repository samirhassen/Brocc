...
public class SystemUserSavingsClient : AbstractSystemUserServiceClient
{
        ...
        IList<Tuple<string, string>> externalVariables)
    {
        return Begin()
            .PostJson("Api/SavingsAccount/Create", new
            {
                ...
                externalVariables =
                    externalVariables?.Select(x => new { Name = x.Item1, Value = x.Item2 }).ToList(),
            })
    ...
    {
        return Begin()
                   .PostJson("Api/SavingsAccount/GetStatusByCustomerIds",
    ...
    {
        var result = Begin()
            .PostJson("Api/InterestRate/FetchCurrentByAccountTypeCode",
                new { savingsAccountTypeCode = "StandardAccount" })
    ...
    public List<FixedRateProduct> GetRatesForFixedInterestAccounts()
    {
        return Begin()
