...
{
    /// <summary>
    /// TODO: This is no longer signicat specific so the name should change
    ///        Beware that the embedded angular apps depend on the current names so make sure to inventory those and change at the same time
    /// </summary>
    public class SignicatController : AnonymousBaseController
    {
        ...
        private bool TryGetAbsoluteLocalUrl(string relativeLocalUrl, NTechServiceRegistry s, out Uri uri)
        {
            ...
            var tempUri = s.External.ServiceUrl("nCustomerPages", relativeLocalUrl);
            if (tempUri.Host != s.External.ServiceRootUri("nCustomerPages").Host) //Guard against any sneaky url edgecases we missed
                return false;
        ...
        public ActionResult CreateLocalLoginSession(string expectedCivicRegNr, string successRedirectUrl, string failedRedirectUrl, string customData)
        {
            ...
            var returnUrl = NEnv.ServiceRegistry.External.ServiceUrl("nCustomerPages", "login/eid/local-api/{localSessionId}/return");
            ...
            var localSession = client.CreateElectronicIdAuthenticationSession(parsedCivicRegNr.NormalizedValue, new Dictionary<string, string>
                {
                    { "Source", "[nCustomerPages]/api/signicatcreate-local-login-session" },
        ...
        public ActionResult CompleteLocalLoginSession(string sessionId, string loginToken)
        {
            ...
            if (!pc.TryGetTemporarilyEncryptedData(sessionId, out var data, removeAfter: true))
            {
            ...
            var key = CreateLoginSessionDataToken(pc, new LoginSessionData
            {
