...
namespace NTech.Core.Module.Shared.Clients
{
    public interface IDocumentClient
    {
        ...
        Task<string> ArchiveStoreWithSourceAsync(byte[] fileData, string mimeType, string filename, string sourceType,
            string sourceId);

        string ArchiveStoreWithSource(byte[] fileData, string mimeType, string filename, string sourceType,
            string sourceId);
        ...
        (bool IsSuccess, List<string> SuccessProfileNames, List<string> FailedProfileNames, int? TimeInMs)
        ...
        string BatchRenderDocumentToArchive(string batchId, string renderedPdfFileName,
            IDictionary<string, object> context);

        void BatchRenderEnd(string batchId);
        byte[] PdfRenderDirect(byte[] template, IDictionary<string, object> context);

        Dictionary<string, List<List<string>>> ParseDataUrlExcelFile(string fileName, string fileAsDataUrl,
        ...
        string BatchRenderDelayedDocumentToArchive(string batchId, string renderedPdfFileName,
            IDictionary<string, object> context);
...
namespace nCredit.Excel
{
    public class DocumentClientExcelRequest
    {
        ...
        public class StyleData
        {
                ...
                IncludeTime
            {
                get;
                set;
            } //When IsDate is true is used to indicate that not just the date, but the date and time should be shown
        ...
        public static List<Tuple<Column, Func<T, object>, Func<T, DocumentClientExcelRequest.StyleData>>>
            CreateDynamicColumnList<T>(IList<T> rows)
        {
        ...
        /// </summary>
        public static DocumentClientExcelRequest CreateSimpleRequest<TItem>(List<TItem> items, string title)
            where TItem : class, new()
        {
            var columns = CreateDynamicColumnList(items);
            foreach (var property in typeof(TItem).GetProperties(System.Reflection.BindingFlags.Instance |
                                                                 System.Reflection.BindingFlags.Public))
            {
                ...
                else if (property.PropertyType == typeof(System.DateTimeOffset?) ||
                         property.PropertyType == typeof(System.DateTimeOffset))
                    columns.Add(items.Col(x => ((DateTimeOffset?)property.GetValue(x))?.DateTime, ExcelType.Date,
                        property.Name));
    ...
    {
        public static Tuple<DocumentClientExcelRequest.Column, Func<T, object>,
            Func<T, DocumentClientExcelRequest.StyleData>> Col<T>(
            this IList<T> source,
        ...
        {
            return new Tuple<DocumentClientExcelRequest.Column, Func<T, object>,
                Func<T, DocumentClientExcelRequest.StyleData>>(new DocumentClientExcelRequest.Column
            {
        ...
        public static void SetColumnsAndData<T>(this DocumentClientExcelRequest.Sheet source, IList<T> rows,
            params Tuple<DocumentClientExcelRequest.Column, Func<T, object>,
                Func<T, DocumentClientExcelRequest.StyleData>>[] columns)
        {
            var actualColumns =
                columns.Where(x => x != null)
                    .ToArray(); //To allow conditionally skipping columns without using CreateDynamicColumnList
            ...
            source.Cells = rows.Select(row => actualColumns.Select(c =>
            {
                ...
                if (style.IsNumber)
                {
                    ...
                    else if (value is int?)
                    {
                        var v = (int?)value;
                        return v.HasValue
                            ? new DocumentClientExcelRequest.Value { V = (decimal)v.Value, Style = so }
                            : new DocumentClientExcelRequest.Value { Style = so };
                    }
