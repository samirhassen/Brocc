...
namespace NTech.Core.Host.IntegrationTests
{
    ...
    {
        private void RepayUnplacedPayment(UlLegacyTestRunner.TestSupport support, BusinessEvent unplacedPaymenEvent)
        {
            ...
            using (var context = new CreditContextExtended(support.CurrentUser, support.Clock))
            {
                ...
                {
                    Assert.That(
                        mgr.TryRepay(unplacedPaymentId1, FirstUnplacedPaymentAmount, 0m, "Test customer", IBANFi.Parse("FI5840550257143115"), context, out var createdRepaymentLocal, out var failedMessage),
                        Is.True);
