...
namespace NTech.Core.Host.Logging
{
    public class NTechHttpRequestLogger : ILogger
    {
        ...
        private readonly RotatingLogFile rotatingLogFile;

        public NTechHttpRequestLogger(bool isEnabled, NEnv env)
        {
            this.isEnabled = isEnabled;
            rotatingLogFile = isEnabled
                ? new RotatingLogFile(Path.Combine(env.LogFolder.FullName, "NTechHostApiLogs"), "ntech-host-api-log-")
                : null;
        }
