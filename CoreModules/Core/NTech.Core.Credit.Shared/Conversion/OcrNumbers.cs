...
namespace nCredit.Code
{
    public class OcrNumberParser
    {
        ...
        private static ConcurrentDictionary<string, OcrNumberParser> parsers = new ConcurrentDictionary<string, OcrNumberParser>();

        public static OcrNumberParser GetSharedInstance(string country) => parsers.GetOrAdd(country, _ => new OcrNumberParser(country));
        public static IOcrNumber Parse(string ocr, string country) => GetSharedInstance(country).Parse(ocr);
