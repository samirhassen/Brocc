    ...
    public class ApiSatExportController : NController
    {
        ...
        private static DeliveryResult DeliverExportFile(string archiveKey, List<string> errors, IDocumentClient dc)
        {
            ...
            if (exportProfileName == null)
            {
                ...
                {
                    SuccessProfileNames = new List<string>(), FailedProfileNames = new List<string>(), TimeInMs = null
                };
            }
            else
            {
                ...
                return new DeliveryResult
                {
                    TimeInMs = exportResult.TimeInMs, SuccessProfileNames = exportResult.SuccessProfileNames,
        ...
        private ActionResult CreateExportI(IDictionary<string, string> schedulerData)
        {
            ...
            try
            {
                using (var context = new CreditContext())
                {
                    ...
                    satFormat.WithTemporaryExportFile(export, tempFileName =>
                    {
                        ...
                        deliveryArchiveKey =
                            dc.ArchiveStoreFile(new FileInfo(tempFileName), "application/xml", "colo.xml");
                        ...
                        var count = (deliveryResult?.FailedProfileNames?.Count ?? 0) +
                                    (deliveryResult?.FailedProfileNames?.Count ?? 0);

                        if (NEnv.SatExportProfileName == null) return;
            ...
            using (var context = new CreditContext())
            {
                context.OutgoingSatExportFileHeaders.Add(new OutgoingSatExportFileHeader
                {
                    ...
                    ExportResultStatus = JsonConvert.SerializeObject(
                        new OutgoingExportFileHeader.ExportResultStatusStandardModel
                        {
