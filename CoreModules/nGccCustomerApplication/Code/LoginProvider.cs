...
namespace nGccCustomerApplication.Code
{
    public class LoginProvider
    {
        ...
        public void SignIn(Microsoft.Owin.IOwinContext context, int customerId, string firstName, bool isStrongIdentity, string authType, NTech.Banking.CivicRegNumbers.ICivicRegNumber civicRegNr, (string Name, string CustomData)? reloginTarget)
        {
        ...
        private void AppendRoleClaims(int customerId, Action<Claim> appendRoleClaim)
        {
            ...
            {
                if (NEnv.IsEmbeddedMortageLoanGccCustomerApplicationEnabled)
                {
                    var c = new CustomerLockedCreditClient(customerId);
                    if (c.HasOrHasEverHadACredit())
            ...
            if (NEnv.IsStandardUnsecuredLoansEnabled || NEnv.IsStandardMortgageLoansEnabled || NEnv.IsGccCustomerApplicationKycQuestionsEnabled)
            {
