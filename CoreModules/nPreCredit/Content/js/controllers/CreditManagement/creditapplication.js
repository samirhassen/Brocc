...
var CreditApplicationCtr = /** @class */ (function () {
    function CreditApplicationCtr($scope, $http, $q, $timeout, trafficCop, ntechComponentService) {
        this.$http = $http;
        this.$q = $q;
        this.$timeout = $timeout;
        $scope.applicationInfo = initialData.ApplicationInfo;
        var ai = $scope.applicationInfo;
        $scope.isApplicationActive = function () { return $scope.applicationInfo.IsActive === true; };
        $scope.isApplicationInactive = function () { return $scope.applicationInfo.IsActive === false; };
        ...
        $scope.additionalQuestionsBlockInitialData = {
            isInitiallyExpanded: ai.IsActive && ai.CreditCheckStatus === 'Accepted' && ai.AgreementStatus !== 'Accepted',
            isActive: ai.IsActive,
            status: ai.AgreementStatus,
            title: 'Agreement'
        };
        $scope.customerCheckBlockInitialData = {
            isInitiallyExpanded: ai.IsActive && ai.CustomerCheckStatus !== 'Accepted' && ai.AgreementStatus === 'Accepted',
            title: 'Customer',
        ...
        $scope.onKycScreenDone = function (customerId) {
            location.reload();
            return false;
        };
        var preCreditApiClient = new NTechPreCreditApi.ApiClient(function (msg) { return toastr.error(msg); }, $http, $q);
        ...
        $scope.isReactivateApplicationAllowed = function (evt) {
            ...
            var isNewCreditCheckAllowed = !($scope.applicationInfo.IsFinalDecisionMade === true || $scope.applicationInfo.AgreementStatus === 'Accepted');
            var isAllowedDueToCancelled = $scope.applicationInfo.IsCancelled === true && $scope.applicationInfo.IsWaitingForAdditionalInformation === false;
        ...
        $scope.rejectApplication = function (evt) {
            ...
            $http({
                ...
                var _a, _b, _c;
                if (((_a = response.data) === null || _a === void 0 ? void 0 : _a.errorCode) && ((_b = response.data) === null || _b === void 0 ? void 0 : _b.errorMessage)) {
                    toastr.warning((_c = response.data) === null || _c === void 0 ? void 0 : _c.errorMessage);
                    reloadComments();
                }
                else {
                    location.reload();
        ...
        $scope.approveApplication = function (evt) {
            ...
            if ($scope.app.ApplicationType === 'mortgageLoan') {
                ...
                preCreditApiClient.approveApplication($scope.app.ApproveApplicationUrl).then(function (result) {
        ...
        $scope.flagCustomersAsExternallyOnboarded = function (evt) {
            if (evt) {
                evt.preventDefault();
            }
            $scope.isLoading = true;
        ...
        if (initialData.IsTest) {
            $scope.onArchive = function (evt) {
                if (evt) {
                    evt.preventDefault();
                }
                preCreditApiClient.archiveSingleApplication(ai.ApplicationNr).then(function () {
    ...
    CreditApplicationCtr.$inject = ['$scope', '$http', '$q', '$timeout', 'trafficCop', 'ntechComponentService'];
    return CreditApplicationCtr;
}());
