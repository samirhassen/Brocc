var decisionDetailsBasisPopupSupport = (function ($http, $scope) {
    ...
    function createDecisionBasisDetails(decisionModel) {
        var r = decisionModel.recommendation;
        var decisionDetailsBasis = {
            ...
            scoringDataModelDetailsInitialData: null
        };
        ...
        if (decisionDetailsBasis.creditReportsUsed) {
            var _loop_1 = function (c) {
                c.onExpanded = function (service) {
                    unlockCreditReportItems(c, null);
                    service.setLocked(false);
                };
            };
            for (var _i = 0, _a = decisionDetailsBasis.creditReportsUsed; _i < _a.length; _i++) {
                var c = _a[_i];
                _loop_1(c);
            }
        }
        if (decisionModel.recommendation && decisionModel.recommendation.ScoringData) {
            var data = decisionModel.recommendation.ScoringData;
    ...
    function createSatFiCreditReportItems(items) {
        var newItems = [];
