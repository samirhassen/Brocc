...
var CreditApplicationEditCtr = /** @class */ (function () {
    function CreditApplicationEditCtr($scope, //ng.IScope
    $http, $q, $timeout) {
        this.$http = $http;
        this.$q = $q;
        ...
        if (booleanFieldNames.some(function (x) { return x === initialData.name; })) {
            $scope.editMode = 'boolean';
            $scope.edit.value = ($scope.edit.value === 'true' || $scope.edit.value === true);
        }
        else if (phoneNrFieldNames.some(function (x) { return x === initialData.name; })) {
            $scope.editMode = 'phonenr';
        }
        else if (positiveIntFieldNames.some(function (x) { return x === initialData.name; })) {
            $scope.editMode = 'positiveInt';
        }
        else if (validMonthFieldNames.some(function (x) { return x === initialData.name; })) {
            $scope.editMode = 'validMonth';
        }
        else if (pulldownFieldNames.some(function (x) { return x === initialData.name; })) {
    ...
    CreditApplicationEditCtr.$inject = ['$scope', '$http', '$q', '$translate', '$timeout'];
    return CreditApplicationEditCtr;
}());
