...
var CreditApplicationsToApproveCtr = /** @class */ (function () {
    function CreditApplicationsToApproveCtr($scope, //ng.IScope
    $http, $q, $timeout) {
        this.$http = $http;
        this.$q = $q;
        ...
        $scope.totalAmountToApprove = function () {
            return _.reduce($scope.approvedApps(), function (memo, x) { return memo + x.amount; }, 0);
        };
        ...
        $scope.filterHistory = function (evt) {
            ...
            $scope.isLoading = true;
            client.findHistoricalDecisions($scope.historyFromDate, $scope.historyToDate).then(function (result) {
                $scope.isLoading = false;
        ...
        $scope.loadBatchDetails = function (b, evt) {
            ...
            $scope.isLoading = true;
            client.getBatchDetails(b.Id).then(function (result) {
                $scope.isLoading = false;
    ...
    CreditApplicationsToApproveCtr.$inject = ['$scope', '$http', '$q', '$timeout'];
    return CreditApplicationsToApproveCtr;
}());
