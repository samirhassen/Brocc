...
app.controller('ctr', ['$scope', '$http', '$q', '$translate', 'modalDialogService', function ($scope, $http, $q, $translate, modalDialogService: ModalDialogComponentNs.ModalDialogService) {
    ...
    $scope.satUi = {}
    let reportsModel = [{ applicationNrAndApplicantNr: { applicationNr: initialData.applicationNr, applicantNr: 1 }, customerId: null, creditReportProviderName: initialData.creditReportProviderName, listProviders: initialData.listCreditReportProviders }]
    ...
    $scope.getManualControlReasonDisplayText = function (code: string) {
    ...
    $scope.updateSatUi = function (applicantNr, evt) {
        ...
        var satReport = _.find($scope.decisionModel.creditReportsUsed, (x: any) => x.ProviderName === 'SatFi' && x.ApplicantNr === applicantNr)
        if (satReport) {
            $scope.satUi['applicant' + applicantNr] = { isLoading: true }
            var addReportItem = function (items, name, target) {
                var i = _.find(items, (x: any) => x.Name === name)
                if (i) {
    ...
    $scope.wasPetrusUsed = () => {
        let petrusOneUsed = !!$scope?.decisionModel?.recommendation?.ScoringData?.Items?.find(x => x?.Name === 'isAcceptedByPetrus');
        let petrusTwoUsed = !!$scope?.decisionModel?.recommendation?.PetrusApplicationId;
        return petrusOneUsed || petrusTwoUsed;
    };

    $scope.showPetrusLog = () => {
        let apiClient = new NTechPreCreditApi.ApiClient(null, $http, $q);
        apiClient.loggingContext = 'viewCreditCheck';
        apiClient.rejectWithFullError = true;
        
        apiClient.postUsingApiGateway('NTechHost', 'Api/PreCredit/Petrus/Audit-Trail', { applicationId: $scope.decisionModel.recommendation.PetrusApplicationId }).then(x => {
            $scope.petrusLogXml = x;
            modalDialogService.openDialog($scope.petrusDialogId)
        }, err => {
            if (err?.data?.errorCode === 'auditTrailMissing') {
