...
namespace nBackOffice.Controllers
{
    public class SecureController : NController
    {
        ...
        {
            var p = this.User?.Identity as ClaimsIdentity;
            if (p != null)
            {
        ...
        {
            using (var p = Serilog.Context.LogContext.PushProperties(NTechLoggingMiddleware.GetProperties(this?.Request?.GetOwinContext()).ToArray()))
            {
        ...
        {
            var ms = MenuWithSubGroups;
            var m = ms.Item1;
            var subGroupOrder = ms.Item2;
            ...
            if (!string.IsNullOrWhiteSpace(testRoles) && !NEnv.IsProduction)
            {
                ...
                isUserInRole = x => testRoles.Contains(x);
            }
            else
            {
                var allMenuRoles = new HashSet<string>(m
                    .SelectMany(x => x.Items.Where(y => y.RequiredRoleName != null).Select(y => y.RequiredRoleName))
                    .Concat(m.SelectMany(x => x.SubGroups.SelectMany(y => y.Items.Where(z => z.RequiredRoleName != null).Select(z => z.RequiredRoleName)))));
            ...
            ViewBag.JsonInitialData = Convert.ToBase64String(Encoding.GetEncoding("iso-8859-1").GetBytes(JsonConvert.SerializeObject(model)));
            ViewBag.IsNavPage = true;
            ViewBag.ReleaseNumber = NTechSimpleSettings.GetValueFromClientResourceFile("CurrentReleaseMetadata.txt", "releaseNumber");
        ...
        {
            var s = NTechCache.WithCache("fdc8dacf-c4e5-425d-9c26-5baeaf7ecd5c", TimeSpan.FromMinutes(15), () => new CrossModuleNavigationService());

            if (!s.TryGetUrlFromCrossModuleNavigationToken(targetCode, NEnv.ServiceRegistry, out var url, out var errorMessage, backTargetCode: backTargetCode.NormalizeNullOrWhitespace()))
                return new HttpStatusCodeResult(System.Net.HttpStatusCode.BadRequest, errorMessage);
            else
        ...
        public ActionResult CrossModuleNavigateResolve(string targetCode)
        {
            ...
            if (!s.TryGetUrlFromCrossModuleNavigationToken(targetCode, NEnv.ServiceRegistry, out var url, out var errorMessage, observeTargetModuleName: x => moduleName = x))
                return new HttpStatusCodeResult(System.Net.HttpStatusCode.BadRequest, errorMessage);
            else
            {
                ...
                return new JsonNetActionResult
                {
                    ...
                    {
                        Url = url,
                        LocalEmbeddedBackofficeUrl = localEmbeddedBackofficeUrl //Helps the embedded angular app having to avoid a bunch of reloads due to passing the gateway going between it's own pages
                    }
                };
            }
        }
