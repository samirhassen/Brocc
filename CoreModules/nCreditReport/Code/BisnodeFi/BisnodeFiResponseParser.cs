...
namespace nCreditReport.Code.BisnodeFi
{
    public class BisnodeFiResponseParser
    {
        ...
        private static string IsoNumberToCode(int? nr)
        {
            if (!nr.HasValue)
                return null;

            if (!CountryCodeByCountryNumber.Value.TryGetValue(nr.Value, out var code))
                return "Other_" + nr.Value;

            return code;
        }

        private static string TranslateBricPaymentRemark(string b)
        {
        ...
        private static bool TryParseErrorableSection<T>(object section, StringBuilder errorMessage,
            Action<T> parseSection)
            where T : class
        {
            if (section is Virhe errorSection)
            {
                errorMessage.AppendFormat($";{errorSection.Kuvaus ?? ""};{errorSection.Tyyppi ?? ""}");
                return false;
            }

            if (!(section is T sectionT))
            {
        ...
        private static bool IsPosteRestante(string streetAddress)
        {
            return streetAddress != null && streetAddress.Equals("Poste restante", StringComparison.OrdinalIgnoreCase);
        }

        public Result Parse(SoliditetHenkiloLuottoTiedotResponse r, bool isAddressOnlyRequest)
        {
            var items = new List<Item>();

            void Add(string name, string value)
            {
                if (!string.IsNullOrWhiteSpace(value)) items.Add(new Item { Name = name, Value = value });
            }

            void AddFrom(string name, Func<string> valueFactory)
            {
            ...
            isError = isError || !TryParseErrorableSection<HenkiloTiedot>(r?.HenkiloTiedotResponse?.Item,
                errorMessage, ht =>
                {
                    ...
                    if (!isBrokenAddress)
                    {
                        ...
                        Add("hasDomesticAddress",
                            hasDomesticAddress.HasValue ? hasDomesticAddress.Value ? "true" : "false" : "false");
                        AddFrom("domesticAddressSinceDate", () =>
                        {
                        ...
                        {
                            if (DateTime.TryParseExact(suomeenMuuttopvmRaw, "yyyyMMdd", CultureInfo.InvariantCulture,
                    ...
                    var hasPosteRestanteAddress =
                        (ht?.Henkilo?.PostiOsoite?.Any(x => IsPosteRestante(x.PostiosoiteS)) ?? false) ||
                        IsPosteRestante(streetAddress);
                    ...
                    {
                        Add("personStatus",
                            (ht?.Hakutiedot?.OnnistuikoHaku?.Koodi ?? "0").TrimStart('0') == "2"
                                ? "deactivated"
                                : "nodata");
                    }
                    else if (!string.IsNullOrWhiteSpace(ht?.Henkilo?.Kuolinpvm) ||
                             !string.IsNullOrWhiteSpace(ht?.Henkilo?.Kuolleeksijulistamispvm))
                    {
                        Add("personStatus", "dead");
                    }
                    else if (ht?.Henkilo?.Edunvalvonta == KyllaEiType1.k ||
                             ht?.Henkilo?.Edunvalvonta == KyllaEiType1.K)
                    {
            ...
            {
                isError = isError || !TryParseErrorableSection<LuottoTietoMerkinnat>(
                    r.LuottoTietoMerkinnatResponse?.Item, errorMessage, remarkSection =>
                    {
                        var nrOfPaymentRemarks = remarkSection?.MerkintojenLkm;
                        Add("nrOfPaymentRemarks",
                            nrOfPaymentRemarks?.ToString());
                        Add("hasPaymentRemark",
                            nrOfPaymentRemarks.HasValue ? nrOfPaymentRemarks.Value > 0 ? "true" : "false" : null);
                    });

                isError = isError || !TryParseErrorableSection<YritysYhteysTiedot>(
                    r.YritysYhteydetResponse?.Item, errorMessage, businessSection =>
                    {
                        var hasBusinessConnection = businessSection.OnVastuuHenkilo == KyllaEiType1.k ||
                                                    businessSection.OnVastuuHenkilo == KyllaEiType1.K ||
                                                    businessSection.VastuuHenkiloTieto != null &&
                                                    businessSection.VastuuHenkiloTieto.Length > 0;
            ...
            {
                IsError = isError,
                IsInvalidCredentialsError = isError &&
                                            (message ?? "").Contains(
                                                "Tietoa ei voida toimittaa. Syy: ei palvelun käyttöoikeutta"),
